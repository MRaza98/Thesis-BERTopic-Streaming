...
def save_individual_model(model_results: dict, 
    ...
    """
    run_dir = f"{output_dir}/metric_{model_results['distance_metric']}"
...
def train_and_evaluate_models(train_documents: List[str], 
    ...
    n_components = 11  # Fixed n_components value
    ...
    for metric in distance_metrics:
        print(f"\nTraining model with distance_metric={metric}...")
        start_time = time.time()
        
        try:
            # Initialize UMAP with fixed n_components
            umap_model = UMAP(
                n_components=n_components,
                random_state=42  # Fixed random state
            )
            
            # Initialize HDBSCAN with specific metric
            ...
            print(f"Completed distance_metric={metric}:")
            print(f"Number of topics: {num_topics}")
            print(f"Coherence score: {c_v_score:.4f}")
            print(f"Training time: {training_time:.2f} seconds")
            
        except Exception as e:
            print(f"Error with distance_metric={metric}: {str(e)}")
            continue
