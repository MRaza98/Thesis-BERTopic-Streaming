    ...
    def split_text(self, row: Dict[str, Any], max_tokens: int = 384, overlap_tokens: int = 50) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for start_idx in range(0, total_tokens, stride):
                ...
                # Use German sentence tokenizer for boundaries
                if start_idx > 0 or end_idx < total_tokens:
                    sentences = self.german_tokenizer.tokenize(chunk_text)
                    if len(sentences) > 1:
                        if start_idx > 0:
                            # Keep complete sentences after the first one
                            chunk_text = ' '.join(sentences[1:])
                        if end_idx < total_tokens:
                            # Keep complete sentences up to the last one
                            chunk_text = ' '.join(sentences[:-1])
...
if __name__ == '__main__':
    # Configuration
    config = {
        'input_path': '/home/raza/projects/Streaming-Pipeline-Parliamentary-Debates/thesis/experiments/data/germany_speeches_2010.csv',
        'output_path': '/home/raza/projects/Streaming-Pipeline-Parliamentary-Debates/thesis/experiments/data/nltk_stopwords_preprocessed_germany_speeches_2010.csv',
