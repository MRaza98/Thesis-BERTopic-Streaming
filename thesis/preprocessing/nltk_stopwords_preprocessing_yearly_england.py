...
class MPNetChunker:
    def __init__(self, model_name: str = "sentence-transformers/all-mpnet-base-v2"):
        ...
        # Add string punctuation handling
        self.punct_translator = str.maketrans("", "", string.punctuation)
        # Add tracking for total stopwords removed
        self.total_stopwords_removed = 0
        self.total_documents_processed = 0
        logger.info(f"Initialized MPNet tokenizer: {model_name}")
        logger.info(f"Loaded {len(self.stop_words)} stopwords")

    def remove_stopwords(self, text: str) -> "tuple[str, int]":
        """
        Remove stopwords from text using the same approach as script A.
        ...
        try:
            ...
            # Convert to lowercase (matching script A)
            text = text.lower()
            
            # Remove punctuation using the same translate approach as script A
            ...
            num_stopwords = sum(1 for word in words if word in self.stop_words)
            
            # Remove stopwords using list comprehension (matching script A)
            filtered_words = [word for word in words if word not in self.stop_words]
            
            # Update global statistics
            self.total_stopwords_removed += num_stopwords
    ...
    def split_text(self, row: Dict[str, Any], max_tokens: int = 384, overlap_tokens: int = 50) -> List[Dict[str, Any]]:
        """
        Split text into chunks using MPNet tokenizer with overlap, after removing stopwords.
        Now includes stopword statistics in the output.
        """
        try:
            # Remove stopwords before tokenization using the aligned approach
            text, num_stopwords = self.remove_stopwords(row['text'])
            ...
            if total_tokens <= max_tokens:
                ...
                return [dict(row, 
                           ...
                           stopwords_removed=num_stopwords)]
            ...
            for start_idx in range(0, total_tokens, stride):
                ...
                new_row.update({
                    ...
                    'stopwords_removed': num_stopwords,  # Add stopword count to each chunk
                })
    ...
    def get_statistics(self) -> Dict[str, float]:
        ...
        Returns:
            Dict containing average stopwords per document and total stopwords removed
        """
        ...
        return {
            'avg_stopwords_per_doc': self.total_stopwords_removed / self.total_documents_processed,
            'total_stopwords_removed': self.total_stopwords_removed,
...
def preprocess_file(
    ...
    try:
        ...
        # Get and log stopword statistics
        stats = chunker.get_statistics()
        logger.info("Stopword removal statistics:")
        logger.info(f"- Average stopwords per document: {stats['avg_stopwords_per_doc']:.2f}")
        logger.info(f"- Total stopwords removed: {stats['total_stopwords_removed']}")
...
def batch_preprocess(
    ...
    overlap_tokens: int = 50,
    chunk_size: int = 10000,
    filter_condition: Dict[str, Any] = {'chair': False}
):
    """Process all yearly files in the input directory"""
    ...
    input_files = sorted(input_dir.glob('england_speeches_*.csv'))
    ...
    for input_file in input_files:
        try:
            year = input_file.stem.split('_')[-1]  # Extract year from filename
            output_file = output_dir / f'nltk_stopwords_preprocessed_england_speeches_{year}.csv'
...
if __name__ == '__main__':
    # Configuration
    config = {
        'input_dir': '/home/raza/projects/Streaming-Pipeline-Parliamentary-Debates/data/yearly_england',
        'output_dir': '/home/raza/projects/Streaming-Pipeline-Parliamentary-Debates/thesis/experiments/data/yearly_england',
